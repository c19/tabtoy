...
namespace {{.Namespace}}{{$globalIndex:=.Indexes}}{{$verticalFields:=.VerticalFields}}
{
	...
	// Defined in table: {{.DefinedTable}}
	{{.CSClassHeader}}
	public partial class {{.Name}} : BaseData
	{
	{{if .IsCombine}}
	...
	{{range .Fields}}	
		...
		public {{.TypeNote}} {{.Name}}
        {
            ...
            set
            {
                var oldVal = _{{.Name}};
                _{{.Name}} = value;
                this.EmitFieldChange("_{{.Name}}", oldVal, value);
            }
...
func (self csharpField) TypeNote() string {
	var type_str string
	switch self.Type {
	case model.FieldType_Int32:
		type_str = "int"
	...
	if self.IsRepeated {
		return fmt.Sprintf("List<%s>", type_str)
	}
	return type_str
}

func (self csharpField) TypeCode() string {
	var name string = "_" + self.Name
	var type_str string = self.TypeNote()

	if self.Type == model.FieldType_Struct || self.IsRepeated {
		return fmt.Sprintf("private %s %s = new %s();", type_str, name, type_str)
	}

	return fmt.Sprintf("private %s %s = %s;", type_str, name, wrapCSharpDefaultValue(self.FieldDescriptor))
}
